...
def app():
 #user_input=st.text_input('Enter Start date (yy-mm-dd)',start)
 start = "2015-01-01"
 end = date.today().strftime("%Y-%m-%d")

 #st.title('Market Prediction Zone:-')
 
 user_input=st.text_input('Enter Stock Ticker:','SBI.NS')
# In[7]:
 ...
 fig.layout.update(title_text='Opening stock value Vs closing stock value:-', xaxis_rangeslider_visible=True)
 st.plotly_chart(fig)
# In[8]:
